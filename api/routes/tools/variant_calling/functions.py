...
def align_sequences(ref: str, sample: str) -> Tuple[str, str]:
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            match = score[i-1][j-1] + (match_score if ref[i-1] == sample[j-1] else mismatch_penalty)
    ...
    i, j = m, n
    while i > 0 or j > 0:
        if i > 0 and j > 0 and score[i][j] == score[i-1][j-1] + (match_score if ref[i-1] == sample[j-1] else mismatch_penalty):
